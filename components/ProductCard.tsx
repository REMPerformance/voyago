    ...
    <>
      ...
      <div className="flex items-center gap-3 p-4 sm:p-5">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="truncate font-display text-[1.02rem] font-bold leading-tight text-ink sm:text-[1.15rem]">{t(product.destination)}</h3>
      ...
      {/* Popis */}
      <p className="px-4 text-[0.8rem] leading-relaxed text-ink-soft line-clamp-2 sm:px-5 sm:text-[0.83rem]">{t(product.summary)}</p>

      {/* Parametre */}
      <dl className="mt-3 space-y-1.5 px-4 sm:mt-4 sm:space-y-2 sm:px-5">
      ...
      {/* Päta */}
      <div className="mt-auto flex items-end justify-between gap-3 border-t border-line-soft p-4 pt-3 sm:p-5 sm:pt-4">
        <div>
          ...
          <div className="mt-0.5 flex items-baseline gap-1.5">
            {soon ? (
              ...
              <>
                <span className="whitespace-nowrap font-display text-[1.2rem] font-extrabold leading-none text-ink sm:text-[1.35rem]">{money(price, lang)}</span>
        ...
        {soon ? (
          ...
          <span className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-ink px-4 py-2.5 text-sm font-semibold text-paper transition-all group-hover:gap-2.5 group-hover:bg-navy-soft">
