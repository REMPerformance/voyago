...
export function ProductCard({ product }: { product: Product }) {
  ...
  const inner = (
    <>
      ...
      {/* Hlavička */}
      <div className="flex items-center gap-2.5 p-3.5 sm:gap-3 sm:p-5">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="truncate font-display text-[0.98rem] font-bold leading-tight text-ink sm:text-[1.15rem]">{t(product.destination)}</h3>
      ...
      {/* Popis */}
      <p className="px-3.5 text-[0.76rem] leading-relaxed text-ink-soft line-clamp-2 sm:px-5 sm:text-[0.83rem]">{t(product.summary)}</p>

      {/* Parametre */}
      <dl className="mt-3 space-y-1.5 px-3.5 sm:mt-4 sm:space-y-2 sm:px-5">
      ...
      {/* Päta */}
      <div className="mt-auto flex items-end justify-between gap-2 border-t border-line-soft p-3.5 pt-3 sm:gap-3 sm:p-5 sm:pt-4">
        <div>
          ...
          <div className="mt-0.5 flex items-baseline gap-1.5">
            <span className="font-display text-[1.05rem] font-extrabold leading-none text-ink whitespace-nowrap sm:text-[1.35rem]">{money(price, lang)}</span>
